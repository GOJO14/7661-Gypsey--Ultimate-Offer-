...
constructor() : RecyclerView.Adapter<SliderItemTypeOne>() {
    ...
    fun setModle(cartListViewModel:CartListViewModel){
        this.cartListViewModel = cartListViewModel
    }
    ...
    inner class Product(var repository: Repository, var activity: Activity) {
        ...
        fun addCart(view: View, data: ListData) {
            var customQuickAddActivity = QuickAddActivity(context = activity!!, theme = R.style.WideDialogFull, product_id = data.product!!.id.toString(), repository = repository!!,cartListViewModel = cartListViewModel)
